...
public class JsoupGetIp {
    ...
    public static List<AgencyIp> getIp(String url) {
        ...
        try {
            Document doc = JsoupGetArticleUrl.getDoc(url);
            ...
            Iterator var11 = ips.iterator();

            while (var11.hasNext()) {
                String ip = (String) var11.next();
    ...
    public static void visit(List<String> blogUrls, List<AgencyIp> ipList) {
        int count = 0;
        while (count <= 20000) {
            Iterator var3 = ipList.iterator();
            while (var3.hasNext()) {
                AgencyIp AgencyIp = (AgencyIp) var3.next();
                ...
                try {
                    Iterator var5 = blogUrls.iterator();

                    while (var5.hasNext()) {
                        String blogUrl = (String) var5.next();
                        Thread.sleep(1000L);
                        Document doc = JsoupGetArticleUrl.getDoc(blogUrl);
                        if (doc != null) {
                            ++count;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        String proxyurl = "http://www.youdaili.net/Daili/guonei/36819.html";
        String proxyurl2 = "http://ip.zdaye.com/FreeIPlist.html";
        final List<AgencyIp> ipList = getForeignIp();
        ipList.addAll(getIp(url));
        final List<String> urls = JsoupGetArticleUrl.getCsdnBlogsUrl();
            ...
            (new Timer()).schedule(new TimerTask() {
                public void run() {
                    pool.submit(new Runnable() {
                        public void run() {
                            System.out.println("开启了" + ++count[0]);
                            JsoupGetIp.visit(urls, ipList);
                        }
                    });
                }
            }, 0L, 1200000L);
